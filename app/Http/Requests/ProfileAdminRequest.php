    ...
    public function rules(): array
    {
        return [
            ...
            'email' => [
                'required',
                'email',
                Rule::unique('users', 'email')->ignore(Auth::guard('admin')->user()->id), // Bỏ qua kiểm tra đối với chính bản ghi đang cập nhật
            ],
            'phone' => [
                'required',
                'string',
                new VietnamPhone,
                Rule::unique('users', 'phone')->ignore(Auth::guard('admin')->user()->id), // Bỏ qua kiểm tra đối với chính bản ghi đang cập nhật
            ],
