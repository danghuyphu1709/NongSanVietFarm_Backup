...
class ReportController extends Controller
{
        ...
        if ($statusMap) {
            foreach ($statusMap as $key => $status) {
                ...
                if ($previousCount === 0 && $currentCount > 0) {
                    // Trường hợp không có dữ liệu hôm trước, nhưng có dữ liệu hôm nay
                    $percentageChange = 100;
                } else if ($previousCount === 0) {
                    // Trường hợp không có dữ liệu hôm trước và hôm nay
                    $percentageChange = 0;
                } else {
                    $percentageChange = (($currentCount - $previousCount) / $previousCount) * 100;
                }

                // Giới hạn phần trăm thay đổi không vượt quá 100%
    ...
    private function calculateRevenueChange($currentData, $previousData)
    {
        ...
        $percentageChange = (($currentData - $previousData) / $previousData) * 100;
