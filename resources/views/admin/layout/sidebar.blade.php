...
<aside id="sidebar" class="sidebar">
    <ul class="sidebar-nav" id="sidebar-nav">
        @foreach (config('sidebar') as $key => $item)
            @php
                ...
                if (isset($item['subtitles']) && !empty($item['subtitles'])) {
                    foreach ($item['subtitles'] as $subtitle) {
                        if (\Auth::guard('admin')->user()->hasAnyRole($subtitle['roles']) && request()->routeIs($subtitle['route'])) {
                            $isActive = true;
                            break;
                        }
                    }
            ...
            @if (\Auth::guard('admin')->user()->hasAnyRole($item['roles']))
                <li class="nav-item">
                    ...
                    @if(isset($item['subtitles']) && !empty($item['subtitles']))
                        <ul id="{{ 'attribute' . $key }}" 
                            ...
                            @foreach($item['subtitles'] as $subtitle)
                                @if (\Auth::guard('admin')->user()->hasAnyRole($subtitle['roles']))
                                    <li>
