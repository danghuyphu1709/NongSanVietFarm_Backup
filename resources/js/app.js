...
console.log("events");
Echo.channel(`notifications`).listen("SystemNotificationEvent", (event) => {
    console.log("events");
    ...
    let total_notificationNew = document.getElementById(
        "total-notification-new"
    );
...
function newNotification(notification, data) {
    ...
    html = html
        .replace(":order_id", element.order_id)
        .replace(":notification_id", data.id);
